class skeet_Sandbag_D {
    objects[] =
  {
    {"Land_BagFence_Short_F", 5}
    ,{"Land_BagFence_Long_F", 10}
    ,{"Land_BagFence_Round_F", 10}
    ,{"Land_BagFence_Corner_F", 5}
    ,{"Land_BagFence_End_F", 5}
    ,{"Land_WoodenShelter_01_F", 20}
    ,{"Land_BagBunker_Small_F", 25}
    ,{"Land_BagBunker_Large_F", 50}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
    ,{"Land_Razorwire_F", 15}
  };
};
  class skeet_HBarrier_D {
  objects[] =
  {
    {"Land_BagFence_short_F", 5}
    ,{"Land_BagFence_long_F", 10}
    ,{"Land_BagFence_round_F", 10}
    ,{"Land_BagFence_corner_F", 5}
    ,{"Land_BagFence_end_F", 5}
    ,{"Land_WoodenShelter_01_F", 20}
    ,{"Land_BagBunker_small_F", 25}
    ,{"Land_BagBunker_large_F", 50}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
    ,{"Land_Razorwire_F", 15}
    ,{"Land_HBarrierTower_F", 100}
    ,{"Land_BagBunker_Tower_F", 100}
    ,{"Land_HBarrier_1_F", 5}
    ,{"Land_HBarrier_3_F", 20}
    ,{"Land_HBarrier_5_F", 20}
    ,{"Land_HBarrier_Big_F", 20}
    ,{"Land_HBarrierWall4_F", 25}
    ,{"Land_HbarrierWall6_F", 25}
    ,{"Land_HBarrierWall_corner_F", 25}
  };
};
  class skeet_Sandbag_W {
  objects[] =
  {
    {"Land_BagFence_01_short_green_F", 5}
    ,{"Land_BagFence_01_long_green_F", 10}
    ,{"Land_BagFence_01_round_green_F", 10}
    ,{"Land_BagFence_01_corner_green_F", 5}
    ,{"Land_BagFence_01_end_green_F", 5}
    ,{"Land_WoodenShelter_01_F", 20}
    ,{"Land_BagBunker_01_small_green_F", 25}
    ,{"Land_BagBunker_01_large_green_F", 50}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
    ,{"Land_Razorwire_F", 15}
  };
};
  class skeet_HBarrier_W {
  objects[] =
  {
    {"Land_BagFence_01_short_green_F", 5}
    ,{"Land_BagFence_01_long_green_F", 10}
    ,{"Land_BagFence_01_round_green_F", 10}
    ,{"Land_BagFence_01_corner_green_F", 5}
    ,{"Land_BagFence_01_end_green_F", 5}
    ,{"Land_WoodenShelter_01_F", 20}
    ,{"Land_BagBunker_01_small_green_F", 25}
    ,{"Land_BagBunker_01_large_green_F", 50}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
    ,{"Land_Razorwire_F", 15}
    ,{"Land_HBarrier_01_big_tower_green_F", 100}
    ,{"Land_HBarrier_01_tower_green_F", 100}
    ,{"Land_HBarrier_01_line_1_green_F", 5}
    ,{"Land_HBarrier_01_line_3_green_F", 20}
    ,{"Land_HBarrier_01_line_5_green_F", 20}
    ,{"Land_HBarrier_01_big_4_green_F", 20}
    ,{"Land_HBarrier_01_wall_4_green_F", 25}
    ,{"Land_HBarrier_01_wall_6_green_F", 25}
    ,{"Land_HBarrier_01_wall_corner_green_F", 25}
  };
};
  class skeet_Trench_Dirt {
  objects[] =
  {
    {"Land_Trench_01_grass_F", 5}
    ,{"Land_Trench_01_forest_F", 5}
    ,{"Land_TrenchFrame_01_F", 5}
    ,{"Land_WoodenShelter_01_F", 20}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
    ,{"Land_Razorwire_F", 15}
  };
};
  class skeet_Trash {
      objects[] =
  {
    {"Land_PortableLight_single_F", 5}
    ,{"Land_PortableLight_double_F", 5}
    ,{"Land_LampHalogen_F", 5}
    ,{"RoadCone_L_F", 5}
    ,{"RoadCone_F", 5}
    ,{"Land_Camping_Light_F", 5}
    ,{"Land_Target_Dueling_01_F", 5}
    ,{"TargetP_Inf_F", 5}
    ,{"TargetP_Inf2_F", 5}
    ,{"plp_bo_RadioSixties", 5}
    ,{"Windsock_01_F", 5}
    ,{"PortableHelipadLight_01_green_F", 5}
    ,{"Land_CampingChair_V1_F", 5}
    ,{"Land_Misc_deerstand", 25}
    ,{"Land_Plank_01_4m_F", 5}
    ,{"Land_Plank_01_8m_F", 10}
  };
};
